...
import { badRequest, serverError } from '@/presentation/controllers/utils'
import { MissingParamError } from '@/presentation/controllers/errors'

export class WebController {
  constructor (private readonly controllerOp: ControllerOperation) {
    this.controllerOp = controllerOp
  }

  public async handle (request: HttpRequest): Promise<HttpResponse> {
    try {
      const missingParams: string = WebController
        .getMissingParams(request, this.controllerOp.requiredParams)
      if (missingParams) return badRequest(new MissingParamError(missingParams))
